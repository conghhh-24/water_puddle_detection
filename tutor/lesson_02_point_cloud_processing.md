...
```
### 笔记
### 1.函数目标：在路面点云高程数据中，找出那些明显偏离正常路面高度的点（比如飞点、噪声、障碍物），以便后续剔除或修复。
### 2.valid_mask = data != 0  表示只取data中为非零的点;
### 3.函数目标：在路面点云高程数据中，将异常值（飞点、噪声、障碍物）替换为中值或均值。
...
- [📝] 可视化至少3个不同路段的数据
### 文件结构
##### 根组（Group）: road_segments
-描述属性: "路面三维高程矩阵集合"  
-单位属性: "mm"（毫米）  
##### 数据集（Datasets）: 共 20 个路段数据
-命名格式：road_segments/0001 到 road_segments/0020  
-每个数据集形状: (101, 41) → 表示 101 行 × 41 列的高程栅格  
-数据类型: float64（双精度浮点数）  
-元数据信息：  
  - 平均有效点比例: 92.1% ± 2.1%
  - 平均高程: 26.6 ± 13.6
